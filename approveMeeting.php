...
$updateQuery = $mysqli->prepare("UPDATE meetings SET meetingAprroved = 'Approved' WHERE meeting_id = ?");
if (!$updateQuery) {
...
if ($updateQuery->execute()) {
    echo json_encode(['status' => 'success', 'message' => 'Meeting approved successfully.']);
} else {
    echo json_encode(['status' => 'error', 'message' => 'Failed to approve meeting.']);
}
